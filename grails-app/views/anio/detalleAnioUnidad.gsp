...
<table class="table table-condensed table-bordered table-striped">
    ...
    <tbody>
        <g:set var="totCantProy" value="0"/>
        <g:set var="totProy" value="0"/>
        <g:set var="totCantCor" value="0"/>
        <g:set var="totCor" value="0"/>
        <g:set var="totCantObraProy" value="0"/>
        <g:set var="totObraProy" value="0"/>
        <g:set var="totCantObraCor" value="0"/>
        <g:set var="totObraCor" value="0"/>
        <g:each in="${datos}" var="dato" status="i">
            <g:set var="totCantProy" value="${totCantProy.toInteger() + dato.value.get(1)}"/>
            <g:set var="totProy" value="${totProy.toDouble() + dato.value.get(0)}"/>
            <g:set var="totCantCor" value="${totCantCor.toInteger() + dato.value.get(5)}"/>
            <g:set var="totCor" value="${totCor.toDouble() + dato.value.get(4)}">/</g:set>
            <g:set var="totCantObraProy" value="${totCantObraProy.toInteger() + dato.value.get(3)}"/>
            <g:set var="totObraProy" value="${totObraProy.toDouble() + dato.value.get(2)}"/>
            <g:set var="totCantObraCor" value="${totCantObraCor.toInteger() + dato.value.get(7)}"/>
            <g:set var="totObraCor" value="${totObraCor.toDouble() + dato.value.get(6)}"/>
            <tr>
                <td>${dato.key}</td>
                        ...
                        if (result) {
                            ...
                            $.ajax({
                                ...
                                success : function (msg) {
                                    if (msg != "no") {
                                        window.location.href = "${createLink(action:'vistaAprobarAnioUnidad')}"
                                    } else {
                                        log("Error interno")
                                    }
                                }
